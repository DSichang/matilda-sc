...
Importing this package is side-effect free (no argument parsing, no training).
The command-line entry points live behind ``if __name__ == "__main__"`` guards
in each ``main_matilda_*`` module.
...
A higher-level object API (``matilda.train`` / ``matilda.task`` taking AnnData
and returning result objects) is being added in ``matilda.api`` / ``matilda.io``.
"""
...
from .api import train, task, TrainResult, TaskResult
