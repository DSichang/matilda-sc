...
(`run_matilda_rna_train()` / `run_matilda_rna_task()` expose them as CLI entry points). Pass the
...
| `matilda_task_files` / file-path inputs | engine `main_train(...)` / `main_task(...)` (raw, file paths) |
...
```python
res = matilda.classify(query, model=fit,                       # query: AnnData or {"rna","adt","atac"}
                       reference={"rna": rna, "adt": adt, "atac": atac},
                       labels="cell_type", query_labels="cell_type")
res.predictions            # query cell-type predictions
res.retrained              # True if it reconciled features + retrained, False if it reused the model
res.common_features        # per-modality feature counts actually used
```
```r
