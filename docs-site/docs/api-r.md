...
| `label` | optional cell-type labels for the report's ground-truth column |
| `assay`, `adt_exp`, `atac_exp` | assay / `altExp` selectors |
| `device` | one of `"auto"`, `"cpu"`, `"cuda"` |
...
| `reference` | a trained object/`matilda_model` to use; `NULL` ⇒ use `x`'s own model. Non-`NULL` ⇒ query mode |
| `label` | cell-type labels; falls back to the model's training labels (required, errors if none can be resolved) |
...
| `label` | cell-type labels; falls back to the model's training labels |
| `assay`, `adt_exp`, `atac_exp` | assay / `altExp` selectors |
| `device` | one of `"auto"`, `"cpu"`, `"cuda"` |
