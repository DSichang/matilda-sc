...
=== "Python"
    ...
    For the I/O readers used in the tutorial (`io.from_10x`, reading `.h5ad`), also install
    `anndata` and `scanpy`.
...
=== "R"
    ...
    **unchanged** PyTorch code and exposes the object API on a SingleCellExperiment. The
    ...
    # or via devtools
    devtools::install_github("DSichang/matilda-r")
    ```
    ...
    - **Suggests**: `testthat`, `knitr`, `rmarkdown`, `BiocFileCache`, and `Seurat`
      (Seurat is suggested for some data loaders)
