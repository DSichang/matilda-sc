...
so "Open in Colab -> Run all" works on a fresh machine. They are written into the
**public package repos** (matilda-docs is private, and Colab can only open notebooks
from public GitHub repos):
  - <matilda-sc>/colab/tutorial-python.ipynb     = docs/tutorial-python.ipynb + pip-install cell
...
DOCS = ROOT / "docs"
# Hosted in the public package repos (sibling checkouts) so Colab can fetch them:
PY_OUT = ROOT.parent.parent / "matilda-sc" / "colab" / "tutorial-python.ipynb"
R_OUT = ROOT.parent.parent / "matilda-r" / "matilda" / "inst" / "colab" / "tutorial-r.ipynb"
...
PY_INSTALL = (
    ...
    '%pip install -q "git+https://github.com/DSichang/matilda-sc.git" "captum==0.7.0" anndata scanpy'
)
